...
for (const slug of ['kingdom-key', 'oblivion', 'ever-after']) {
  await p.goto(`http://127.0.0.1:3014/fr/encyclopedie/keyblades/${slug}`, { waitUntil: 'networkidle' });
  ...
  if (slug === 'kingdom-key') {
    ...
    await sec.asElement().screenshot({ path: '/tmp/kb-stats.png' });
  }
}
await b.close();
