...
export function EntryCard({ entry, kindLabel }: { entry: Entry; kindLabel: string }) {
  ...
  const source = entry.category === "characters" && entry.source ? (locale === "fr" ? entry.source.fr : entry.source.en) : undefined;
  return (
      ...
      <Link href={`/encyclopedie/${CATEGORY_SLUG[entry.category]}/${entry.slug}`} className="flex h-full items-center gap-4 p-3">
        ...
        <div className="min-w-0">
          <span className="eyebrow">
            {kindLabel}
            {source && <span className="normal-case tracking-normal text-text-2"> · {source}</span>}
          </span>
